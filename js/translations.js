...
window.simple_mirror_translations = {
  en: {
    ...
    description_html: `<p><em>${translation_app_names.en}</em> is a utility based on <a href="${translations_rsync_url}" target="_blank" rel="noreferrer">rsync</a> to create and update back-up copies in few clicks.</p><ul><li>Choose an origin folder;</li><li>Connect an external drive as the destination;</li><li>Synchronize to turn the destination into a mirror of the origin.</li></ul>`,
  ...
  it: {
    ...
    description_html: `<p><em>${translation_app_names.it}</em> è un'utilità basata su <a href="${translations_rsync_url}" target="_blank" rel="noreferrer">rsync</a> per creare e aggiornare copie di backup in pochi clic.</p><ul><li>Scelga una cartella di origine;</li><li>Colleghi un'unità esterna come destinazione;</li><li>Sincronizzi per trasformare la destinazione in uno specchio dell'origine.</li></ul>`,
  ...
  ja: {
    ...
    description_html: `<p><em>${translation_app_names.ja}</em> は <a href="${translations_rsync_url}" target="_blank" rel="noreferrer">rsync</a> をベースにしたユーティリティで、数回のクリックでバックアップのコピーを作成および更新できます。</p><ul><li>コピー元フォルダーを選びます;</li><li>外付けドライブをコピー先として接続します;</li><li>同期してコピー先をコピー元のミラーにします.</li></ul>`,
  ...
  nl: {
    ...
    description_html: `<p><em>${translation_app_names.nl}</em> is een hulpprogramma op basis van <a href="${translations_rsync_url}" target="_blank" rel="noreferrer">rsync</a> om in een paar klikken back-upkopieën te maken en bij te werken.</p><ul><li>Kies een bronmap;</li><li>Sluit een externe schijf aan als bestemming;</li><li>Synchroniseer om de bestemming in een spiegel van de bron te veranderen.</li></ul>`,
  ...
  "zh-CN": {
    ...
    description_html: `<p><em>${translation_app_names["zh-CN"]}</em> 是一个基于 <a href="${translations_rsync_url}" target="_blank" rel="noreferrer">rsync</a> 的工具，可通过几次点击创建和更新备份副本。</p><ul><li>选择源目录;</li><li>将外接驱动器连接为目标目录;</li><li>执行同步，把目标目录变成源目录的镜像.</li></ul>`,
