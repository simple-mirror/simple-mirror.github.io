...
window.simple_mirror_translations = {
  en: {
    ...
    title: "Welcome!",
    meta_description: `${translation_app_names.en} is a reliable utility based on rsync to create and update back-up copies in few clicks.`,
    description_html: `<p><em>${translation_app_names.en}</em> is a reliable utility based on <a href="${translations_rsync_url}" target="_blank" rel="noreferrer">rsync</a> to create and update back-up copies in few clicks.</p><ul><li>Choose an origin folder;</li><li>Connect a back-up medium as the destination;</li><li>Synchronize to turn the destination into a mirror of the origin.</li></ul>`,
    warning_note_html: `The <em>origin</em> and <em>destination</em> folders must be correctly specified. If not, <span class="warning_danger">THERE WILL BE DATA LOSS!</span>`,
  ...
  de: {
    ...
    title: "Willkommen!",
    meta_description: `${translation_app_names.de} ist ein zuverlässiges Dienstprogramm auf Basis von rsync, um Back-up-Kopien mit wenigen Klicks zu erstellen und zu aktualisieren.`,
    description_html: `<p><em>${translation_app_names.de}</em> ist ein zuverlässiges Dienstprogramm auf Basis von <a href="${translations_rsync_url}" target="_blank" rel="noreferrer">rsync</a>, um Back-up-Kopien mit wenigen Klicks zu erstellen und zu aktualisieren.</p><ul><li>Wählen Sie einen Quellordner;</li><li>Verbinden Sie ein Back-up-Medium als Ziel;</li><li>Synchronisieren Sie, um das Ziel in ein Spiegelbild der Quelle zu verwandeln.</li></ul>`,
    warning_note_html: `Die Ordner für <em>Quelle</em> und <em>Ziel</em> müssen korrekt angegeben werden. Andernfalls <span class="warning_danger">GEHEN DATEN VERLOREN!</span>`,
  ...
  es: {
    ...
    title: "¡Bienvenido!",
    meta_description: `${translation_app_names.es} es una utilidad fiable basada en rsync para crear y actualizar copias de seguridad en pocos clics.`,
    description_html: `<p><em>${translation_app_names.es}</em> es una utilidad fiable basada en <a href="${translations_rsync_url}" target="_blank" rel="noreferrer">rsync</a> para crear y actualizar copias de seguridad en pocos clics.</p><ul><li>Elija una carpeta de origen;</li><li>Conecte un medio de copia de seguridad como destino;</li><li>Sincronice para convertir el destino en un espejo del origen.</li></ul>`,
    warning_note_html: `Las carpetas de <em>origen</em> y <em>destino</em> deben especificarse correctamente. De lo contrario, <span class="warning_danger">¡HABRÁ PÉRDIDA DE DATOS!</span>`,
  ...
  fr: {
    ...
    title: "Bienvenue !",
    meta_description: `${translation_app_names.fr} est un utilitaire fiable basé sur rsync pour créer et mettre à jour des copies de sauvegarde en quelques clics.`,
    description_html: `<p><em>${translation_app_names.fr}</em> est un utilitaire fiable basé sur <a href="${translations_rsync_url}" target="_blank" rel="noreferrer">rsync</a> pour créer et mettre à jour des copies de sauvegarde en quelques clics.</p><ul><li>Choisissez un dossier source;</li><li>Branchez un support de sauvegarde comme destination;</li><li>Synchronisez pour transformer la destination en miroir de la source.</li></ul>`,
    warning_note_html: `Les dossiers <em>source</em> et <em>destination</em> doivent être correctement indiqués. Sinon, <span class="warning_danger">IL Y AURA UNE PERTE DE DONNÉES !</span>`,
  ...
  it: {
    ...
    title: "Benvenuto!",
    meta_description: `${translation_app_names.it} è un'utilità affidabile basata su rsync per creare e aggiornare copie di backup in pochi clic.`,
    description_html: `<p><em>${translation_app_names.it}</em> è un'utilità affidabile basata su <a href="${translations_rsync_url}" target="_blank" rel="noreferrer">rsync</a> per creare e aggiornare copie di backup in pochi clic.</p><ul><li>Scelga una cartella di origine;</li><li>Colleghi un supporto di backup come destinazione;</li><li>Sincronizzi per trasformare la destinazione in uno specchio dell'origine.</li></ul>`,
    warning_note_html: `Le cartelle di <em>origine</em> e <em>destinazione</em> devono essere specificate correttamente. In caso contrario, <span class="warning_danger">CI SARÀ UNA PERDITA DI DATI!</span>`,
  ...
  ja: {
    ...
    language_label: "言語:",
    title: "ようこそ!",
    meta_description: `${translation_app_names.ja} は rsync をベースにした信頼できるユーティリティで、数回のクリックでバックアップのコピーを作成および更新できます。`,
    description_html: `<p><em>${translation_app_names.ja}</em> は <a href="${translations_rsync_url}" target="_blank" rel="noreferrer">rsync</a> をベースにした信頼できるユーティリティで、数回のクリックでバックアップのコピーを作成および更新できます。</p><ul><li>コピー元フォルダーを選びます;</li><li>バックアップ用メディアをコピー先として接続します;</li><li>同期してコピー先をコピー元のミラーにします.</li></ul>`,
    warning_note_html: `<em>コピー元</em> フォルダーと <em>コピー先</em> フォルダーは正しく指定する必要があります。そうしないと、<span class="warning_danger">データが失われます！</span>`,
  ...
  nl: {
    ...
    language_label: "Taal:",
    title: "Welkom!",
    meta_description: `${translation_app_names.nl} is een betrouwbaar hulpprogramma op basis van rsync om in een paar klikken back-upkopieën te maken en bij te werken.`,
    description_html: `<p><em>${translation_app_names.nl}</em> is een betrouwbaar hulpprogramma op basis van <a href="${translations_rsync_url}" target="_blank" rel="noreferrer">rsync</a> om in een paar klikken back-upkopieën te maken en bij te werken.</p><ul><li>Kies een bronmap;</li><li>Sluit een back-upmedium aan als bestemming;</li><li>Synchroniseer om de bestemming in een spiegel van de bron te veranderen.</li></ul>`,
    warning_note_html: `De mappen voor <em>bron</em> en <em>bestemming</em> moeten correct worden opgegeven. Zo niet, dan <span class="warning_danger">GAAN ER GEGEVENS VERLOREN!</span>`,
  ...
  pt: {
    ...
    language_label: "Idioma:",
    title: "Bem-vindo!",
    meta_description: `${translation_app_names.pt} é um utilitário fiável baseado em rsync para criar e atualizar cópias de segurança em poucos cliques.`,
    description_html: `<p><em>${translation_app_names.pt}</em> é um utilitário fiável baseado em <a href="${translations_rsync_url}" target="_blank" rel="noreferrer">rsync</a> para criar e atualizar cópias de segurança em poucos cliques.</p><ul><li>Escolha uma pasta de origem;</li><li>Ligue um suporte de cópia de segurança como destino;</li><li>Sincronize para transformar o destino num espelho da origem.</li></ul>`,
    warning_note_html: `As pastas de <em>origem</em> e <em>destino</em> devem ser corretamente especificadas. Caso contrário, <span class="warning_danger">HAVERÁ PERDA DE DADOS!</span>`,
  ...
  "zh-CN": {
    ...
    language_label: "语言：",
    title: "欢迎！",
    meta_description: `${translation_app_names["zh-CN"]} 是一个基于 rsync 的可靠工具，可通过几次点击创建和更新备份副本。`,
    description_html: `<p><em>${translation_app_names["zh-CN"]}</em> 是一个基于 <a href="${translations_rsync_url}" target="_blank" rel="noreferrer">rsync</a> 的可靠工具，可通过几次点击创建和更新备份副本。</p><ul><li>选择源目录;</li><li>将备份介质连接为目标目录;</li><li>执行同步，把目标目录变成源目录的镜像.</li></ul>`,
    warning_note_html: `<em>源目录</em>和<em>目标目录</em>必须正确指定。否则，<span class="warning_danger">将会发生数据丢失！</span>`,
