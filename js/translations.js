...
window.simple_mirror_translations = {
  ...
  nl: {
    ...
    description_html: `<p><em>${translation_app_names.nl}</em> is een hulpprogramma op basis van <a href="${translations_rsync_url}" target="_blank" rel="noreferrer">rsync</a> om in een paar klikken back-upkopieën te maken en bij te werken.</p><ul><li>Kies een bronmap;</li><li>Sluit een externe schijf aan als bestemming;</li><li>Synchroniseer om de bestemming in een spiegel van de bron te veranderen.</li></ul><p>De mappen voor <em>bron</em> en <em>bestemming</em> moeten correct worden opgegeven. Zo niet, dan <span class="warning_danger">GAAN ER GEGEVENS VERLOREN!</span></p>`,
  ...
  "zh-CN": {
    ...
    description_html: `<p><em>${translation_app_names["zh-CN"]}</em> 是一个基于 <a href="${translations_rsync_url}" target="_blank" rel="noreferrer">rsync</a> 的工具，可通过几次点击创建和更新备份副本。</p><ul><li>选择源目录;</li><li>将外接驱动器连接为目标目录;</li><li>执行同步，把目标目录变成源目录的镜像.</li></ul><p><em>源目录</em>和<em>目标目录</em>必须正确指定。否则，<span class="warning_danger">将会发生数据丢失！</span></p>`,
