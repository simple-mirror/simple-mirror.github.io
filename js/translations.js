...
window.simple_mirror_translations = {
  ...
  de: {
    ...
    description_html: `<p><em>${translation_app_names.de}</em> ist ein zuverlässiges Dienstprogramm auf Basis von <a href="${translations_rsync_url}" target="_blank" rel="noreferrer">rsync</a>, um Back-up-Kopien mit wenigen Klicks zu erstellen und zu aktualisieren.</p><ul><li>Wählen Sie einen Ursprungsordner;</li><li>Verbinden Sie ein Back-up-Medium als Ziel;</li><li>Synchronisieren Sie, um das Ziel in ein Spiegelbild des Ursprungs zu verwandeln.</li></ul>`,
    warning_note_html: `Die Ordner für <em>Ursprung</em> und <em>Ziel</em> müssen korrekt angegeben werden. Andernfalls <span class="warning_danger">GEHEN DATEN VERLOREN!</span>`,
  ...
  it: {
    ...
    description_html: `<p><em>${translation_app_names.it}</em> è un'utilità affidabile basata su <a href="${translations_rsync_url}" target="_blank" rel="noreferrer">rsync</a> per creare e aggiornare copie di backup in pochi clic.</p><ul><li>Scegli una cartella di origine;</li><li>Collega un supporto di backup come destinazione;</li><li>Sincronizza per trasformare la destinazione in uno specchio dell'origine.</li></ul>`,
  ...
  ja: {
    ...
    description_html: `<p><em>${translation_app_names.ja}</em> は <a href="${translations_rsync_url}" target="_blank" rel="noreferrer">rsync</a> をベースにした信頼できるユーティリティで、数回のクリックでバックアップのコピーを作成および更新できます。</p><ul><li>元になるフォルダーを選びます;</li><li>バックアップ用メディアを保存先として接続します;</li><li>同期して保存先を元のフォルダーのミラーにします.</li></ul>`,
    warning_note_html: `<em>元</em> フォルダーと <em>保存先</em> フォルダーは正しく指定する必要があります。そうしないと、<span class="warning_danger">データが失われます！</span>`,
  ...
  "zh-CN": {
    ...
    description_html: `<p><em>${translation_app_names["zh-CN"]}</em> 是一个基于 <a href="${translations_rsync_url}" target="_blank" rel="noreferrer">rsync</a> 的可靠工具，可通过几次点击创建和更新备份副本。</p><ul><li>选择一个源文件夹;</li><li>将备份介质连接为目标位置;</li><li>执行同步，把目标变成源文件夹的镜像.</li></ul>`,
    warning_note_html: `<em>源</em>文件夹和<em>目标</em>文件夹必须正确指定。否则，<span class="warning_danger">将会发生数据丢失！</span>`,
